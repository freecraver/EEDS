...
gap=120 #choose for 30 minute gap
#gap=2874/2 #choose for day gap
...
fit_seasonal<-auto.arima(seasonal_stl_energy[-c(LL:L)],D=1)
...
plot(as.ts(energy$power_clean[-c(LL:L)]))
points(energy$extracted[-c(LL:L)],col="red",pch = ".")
lines(new_forecast,col="blue")
RMSE<-rmse(energy$power_clean[(LL+1):L],new_forecast)
MAPE<-mape(energy$power_clean[(LL+1):L],new_forecast)
end_time = Sys.time()
end_time - start_time
MAPE
RMSE
