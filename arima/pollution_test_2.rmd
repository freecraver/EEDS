...
ind <- which(foo %in% sample(foo, 1000))
...
daily<-timeAverage(pollution.df,avg.time="hour",data.thresh = 100,start.date="2014-08-01 00:05:00", end.date="2014-08-01 01:00:00")
...
ozone_ma72 = ts(na.omit(pollution.df$ozone_ma7), frequency=7)
decomp2 = stl(ozone_ma72, s.window="periodic")
deseasonal_ozone2 <- seasadj(decomp2)

#plot(decomp)
auto.arima(deseasonal_ozone, seasonal=FALSE)
fit<-auto.arima(deseasonal_ozone[-c(14000:15552)], seasonal=FALSE)
fcast <- forecast(fit, h=1552)


plot(decomp)
plot(fcast,main=" ")
lines(ts(deseasonal_ozone))
hold <- window(deseasonal_ozone2, start=1)
...
```{r}
#daily$clean_ozone = tsclean(count_ts)
nitrogen_ma7 = ts(na.omit(pollution.df$nitrogen_ma7), frequency=2016)
decomp = stl(nitrogen_ma7, s.window="periodic")
deseasonal_nitrogen <- seasadj(decomp)

nitrogen_ma72 = ts(na.omit(pollution.df$nitrogen_ma7), frequency=7)
decomp2 = stl(nitrogen_ma72, s.window="periodic")
deseasonal_nitrogen2 <- seasadj(decomp2)

#plot(decomp)
auto.arima(deseasonal_nitrogen, seasonal=FALSE)
fit<-auto.arima(deseasonal_nitrogen[-c(14000:15552)], seasonal=FALSE)
fcast <- forecast(fit, h=1552)


plot(decomp)
plot(fcast,main=" ")
lines(ts(deseasonal_nitrogen))
hold <- window(ts(deseasonal_nitrogen2), start=1)
```
...
```{r}
#daily$clean_ozone = tsclean(count_ts)
particullate_ma7 = ts(na.omit(pollution.df$particullate_ma7), frequency=2016)
decomp = stl(particullate_ma7, s.window="periodic")
deseasonal_particullate <- seasadj(decomp)

particullate_ma72 = ts(na.omit(pollution.df$particullate_ma7), frequency=7)
decomp2 = stl(particullate_ma72, s.window="periodic")
deseasonal_particullate2 <- seasadj(decomp2)

#plot(decomp)
auto.arima(deseasonal_particullate, seasonal=FALSE)
fit<-auto.arima(deseasonal_particullate[-c(14000:15552)], seasonal=FALSE)
fcast <- forecast(fit, h=1552)


plot(decomp)
plot(fcast,main=" ")
lines(ts(deseasonal_particullate))
hold <- window(ts(deseasonal_particullate2), start=1)
```
...
fit<-auto.arima(deseasonal_sulfure[-c(47:56)], seasonal=FALSE)
fcast <- forecast(fit, h=10)

plot(decomp)
plot(fcast,main=" ")
lines(ts(deseasonal_sulfure))
...
deseasonal_nitrogen <- seasadj(decomp)
auto.arima(deseasonal_nitrogen, seasonal=FALSE)
fit<-auto.arima(deseasonal_nitrogen[-c(47:56)], seasonal=FALSE)
fcast <- forecast(fit, h=10)
